...
public static class AsyncLauncher
{
    ...
    public static async Task LaunchDispatcher(Action action) => await Dispatcher.UIThread.InvokeAsync(action, DispatcherPriority.Background);
    public static async void LaunchDispatcherVoid(Action action) => await Dispatcher.UIThread.InvokeAsync(action, DispatcherPriority.Background);
}